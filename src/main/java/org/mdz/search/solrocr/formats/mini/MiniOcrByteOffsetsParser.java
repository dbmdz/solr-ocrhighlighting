...
public class MiniOcrByteOffsetsParser {
  ...
  private static final Searcher<SequenceMatcher> BEGIN_SEARCHER =
      new SequenceMatcherSearcher(new ByteSequenceMatcher("<w x=\""));
  private static final Searcher<SequenceMatcher> END_SEARCHER =
  ...
  public static void parse(byte[] ocrBytes, OutputStream os) throws IOException {
    ForwardSearchIterator<SequenceMatcher> beginIt = new ForwardSearchIterator<>(BEGIN_SEARCHER, ocrBytes);
    ForwardSearchIterator<SequenceMatcher> endIt = new ForwardSearchIterator<>(END_SEARCHER, ocrBytes);
  ...
  public static void main(String[] args) throws IOException {
    ...
    parse(Files.readAllBytes(Paths.get("src/test/resources/data/miniocr_utf8.xml")), bos);
